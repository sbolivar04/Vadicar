...
interface AsignarProduccionModalProps {
  isOpen: boolean;
  onClose: () => void;
  pedido: Pedido | null;
...
const AsignarProduccionModal: React.FC<AsignarProduccionModalProps> = ({ isOpen, onClose, pedido, onAsignacionCompleta, labor = 'Confección', etapaDestinoId = null, approvedWorkOrders }) => {
  const { user } = useAuth();
      ...
      const fetchInitialData = async () => {
        ...
        try {
          ...
          if (approvedWorkOrders && approvedWorkOrders.length > 0) {
            ...
            if (labor === 'Confección') {
              query = supabase
                ...
                .select('id_referencia, nombre_referencia, imagen_url, id_talla, nombre_talla, cantidad')
              ...
              query = supabase.rpc('obtener_cantidades_disponibles_para_etapa', {
                ...
                p_nombre_etapa: labor === 'Ojal y botón' ? 'Ojal y botón' : 'Planchado y empaque'
              });
            }
          ...
          if (!detallesData || detallesData.length === 0) {
            throw new Error(`Este pedido no tiene piezas disponibles para asignar a la etapa de ${labor}.`);
          }
  ...
  const handleConfirmarAsignacion = async () => {
    ...
    try {
      const { error: rpcError } = await supabase.rpc('crear_ordenes_de_trabajo', {
        ...
        p_id_etapa: etapaDestinoId // Pasamos la etapa destino
      });
      if (rpcError) {
        ...
        >
          <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col">
            <div className="p-5 border-b dark:border-gray-700 flex justify-between items-center">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">Asignar Producción - Pedido #{pedido.numero_pedido}</h2>
...
export default AsignarProduccionModal;
